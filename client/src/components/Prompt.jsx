...
export const Prompt = ({ option, text, message, action }) => {
  const { popups, setPopups } = useContextConsumer();

  return (
    <Dialog open={popups.logout} onClose={setPopups} className="relative z-10">
      <DialogBackdrop
        transition
      ...
      <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
        <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
          ...
          >
            ...
            <div className="bg-black-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              <button
                type="button"
                onClick={action}
              ...
              <button
                ...
                data-autofocus
                onClick={() =>
                  setPopups((prev) => ({ ...prev, logout: false }))
                }
                className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 outline-none hover:bg-gray-50 sm:mt-0 sm:w-auto"
              >
                Cancel
              </button>
            </div>
