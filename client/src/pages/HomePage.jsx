...
export const HomePage = () => {
  ...
  return (
    <main className="flex">
      <section
        className="w-[70%] flex items-center justify-center min-h-screen"
        id="allPosts"
      >
        <ul id="postList" className="w-[70%] mt-6 space-y-3 mx-auto">
          {posts?.map((post) => (
            <RandomPost key={post._id} post={post} />
          ))}
        </ul>
      </section>
      <section className="w-[30%] bg-green-600" id="suggestions">
        suggestions
      </section>
    </main>
  );
